...
Team は Apple Developer Program のチームを選んでください
（Personal Team では iCloud の権限を付けられません）。
Xcode が権限ファイルを整えてくれます。アプリ側は入っていれば自動で使い、
入っていなければ端末内保存に落ちます。

### 入れる順番に気をつける

食い違ったときは**あとから書いたほうを採る**作りです。つまり
**残したい一覧を持っている端末から先に書かせる**必要があります。

1. **残したい一覧がある端末**（たとえば Mac）で ▶ を押して起動する
2. その端末で地点をひとつ開き、**［保存］を押す**。
   これで iCloud に書き込まれます（起動しただけでは書きません）
3. そのあとで、ほかの端末を ▶ で入れ直す

逆にすると、あとから起動した端末の一覧で上書きされます。

### 確かめ方

画面下の「取得状況」のいちばん下を見てください。

- **「iCloud で iPhone・iPad・Mac に行き来します」** … 効いています
- **「この端末の中だけに保存しています」** … 権限が入っていません

反映には数秒から数分かかります。すぐに出ないときは、
アプリをいったん閉じて開き直してください。
3台とも**同じ Apple ID で iCloud にサインイン**している必要があります。
...
サーバ（`https://weather.shome.co.jp`）が3時間おきにページを読んで
書き出した `data/latest.json` を、アプリが読んでいます。
