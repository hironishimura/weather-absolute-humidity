...
var SOURCES = {
  ...
  ecmwf:       { name: 'ECMWF', short: 'ECMWF', color: '#7B3FA0',
                 about: '欧州中期予報センターの全球モデル（IFS 0.25°）を Open-Meteo 経由で取得。'
                        + '気象庁とは別の計算なので、両者が一致していれば見通しが立ち、'
                        + '割れていれば難しい状況だと分かります。降水確率は出ません。' }
};

/* 画面に出す順番。ここ1か所で決めます（あちこちに並びを書くと足し忘れます） */
var SOURCE_KEYS = ['jma', 'model', 'ecmwf', 'yahoo', 'weathernews'];

/* 降水確率の当たり具合を数えている提供元。ECMWFに降水確率はありません */
var ACC_KEYS = ['jma', 'model', 'yahoo', 'weathernews'];
...
/* 直近の10分値。グラフに出すのは直近1時間ぶんなので、新しい2ファイルで足ります */
...
/* ---------------------------------------------------------
   ...
   --------------------------------------------------------- */
function loadModel(place, key, models) {
  key = key || 'model';
  models = models || 'jma_seamless';
  /* 降水確率は総合予報からしか取れないので、気象庁モデルのときだけ頼みます */
  var wantPop = key === 'model';
  ...
  /* 降水確率は気象庁のモデル（MSM/GSM）には入っていません。
     モデルを指定すると全部 null で返ってくるので、
     Open-Meteo が各社のモデルをまとめて出している値をもらいます。 */
  function popUrl() {
    return OPEN_METEO + '?latitude=' + place.lat.toFixed(4) +
      '&longitude=' + place.lon.toFixed(4) +
  ...
  return Promise.all([
    getJSON(url(true)).catch(function () { return getJSON(url(false)); }),
    wantPop ? getJSON(popUrl()).catch(function () { return null; }) : null
  ]).then(function (res) {
      var d = res[0];
      var pops = {};
      var ph = res[1] && res[1].hourly;
      if (ph && ph.time && ph.precipitation_probability) {
        for (var j = 0; j < ph.time.length; j++) {
          var pv = ph.precipitation_probability[j];
          if (isNum(pv)) { pops[ph.time[j]] = pv; }
        }
      }
      ...
      setStatus(key, true, rows.length + '時間ぶんの予報を読み込みました' +
        (!wantPop ? '（このモデルに降水確率はありません）'
          : Object.keys(pops).length ? '（降水確率は Open-Meteo の総合予報）'
          : '（降水確率は取れませんでした）'));
      return rows;
    })
    .catch(function (e) {
      setStatus(key, false, '取得できませんでした（' + e.message + '）');
    });
}
...
function renderAccuracy() {
  ...
  $('acc-note').textContent =
    ...
    '降水確率の刻みは提供元で違うため（気象庁6時間・Yahoo!天気6時間・' +
    'ウェザーニュース午前午後・数値予報1時間）、その期間に重なる区切りのうち' +
    'いちばん高い値を使っています。刻みが細かいほど高めに出やすい点はご承知おきください。' +
    '気象庁MSM/GSMの降水確率は、気象庁のモデルに含まれていないため ' +
    'Open-Meteo の総合予報の値です。' +
...
function niceScale(min, max, want) {
  ...
  var step = niceStep(max - min, want);
...
/* 表示範囲に入る「日」を並べる */
function daysInRange(xMin, xMax) {
  var out = [];
  var j = new Date(xMin + 9 * 3600e3);
  j.setUTCHours(0, 0, 0, 0);
  var t = j.getTime() - 9 * 3600e3;
  for (var i = 0; i < 20 && t < xMax; i++) {
    var next = t + 24 * 3600e3;
    if (next > xMin) {
      out.push({ t0: t, t1: next, key: jstKey(new Date(t + 3600e3)), date: new Date(t) });
    }
    t = next;
  }
  return out;
...
/* 時刻の目盛りを何時間おきにするか決める。
   表示範囲がせまいほど細かく刻み、入りきらないときは
   1→2→3→6→12→24時間 と順に粗くして重ならないようにします。
   間隔がせまいときは「時」を省いて数字だけにします。

     span   表示している時間の幅（時間）
     usable グラフの目盛りに使える横幅（px） */
function hourTicks(span, usable) {
  var STEPS = [1, 2, 3, 6, 12, 24];
  var i = span > 120 ? 5 : (span > 80 ? 3 : (span > 30 ? 2 : 1));
  while (i < STEPS.length - 1 && usable / (span / STEPS[i]) < 24) { i++; }
  var gap = usable / (span / STEPS[i]);
  return { step: STEPS[i], unit: gap >= 34, gap: gap };
}
...
function lineChart(o) {
  var W = o.width || 720;
  var H = o.height || 150;
  var PL = 46, PR = 14, PT = o.padTop || 14, PB = 42;

  var wrap = el('div', 'chart');
  wrap.appendChild(el('p', 'chart__t', o.title + '（' + o.unit + '）'));
  ...
  (o.marks || []).forEach(function (m) {
    if (isNum(m.max)) { max = Math.max(max, m.max); min = Math.min(min, m.max); }
  ...
  if (o.fromZero) {
    /* 雨量は0から。降っていないときも軸がつぶれないよう、少し余裕を持たせます。 */
    min = 0;
    max = Math.max(max, 1);
  }
  var sc = o.range ? { min: o.range[0], max: o.range[1],
                       ticks: [o.range[0], (o.range[0] + o.range[1]) / 2, o.range[1]] }
                   : niceScale(min, max, 3);

  var svg = svgEl('svg', { viewBox: '0 0 ' + W + ' ' + H, role: 'img', 'aria-label': o.title });
  function x(t) { return PL + ((t - o.xMin) / (o.xMax - o.xMin)) * (W - PL - PR); }
  function y(v) { return PT + (1 - (v - sc.min) / (sc.max - sc.min)) * (H - PT - PB); }

  /* 横の目盛り */
  sc.ticks.forEach(function (v) {
    svg.appendChild(svgEl('line', { class: 'grid', x1: PL, x2: W - PR, y1: y(v), y2: y(v) }));
    var tx = svgEl('text', { class: 'axis', x: PL - 8, y: y(v) + 4, 'text-anchor': 'end' });
    tx.textContent = v.toFixed(o.digits);
    svg.appendChild(tx);
  });

  /* 日の区切りと日付 */
  var days = daysInRange(o.xMin, o.xMax);
  days.forEach(function (d) {
    if (d.t0 > o.xMin) {
      svg.appendChild(svgEl('line', {
        class: 'day-line', x1: x(d.t0), x2: x(d.t0), y1: PT, y2: H - PB + 16
      }));
    }
    var mid = (Math.max(d.t0, o.xMin) + Math.min(d.t1, o.xMax)) / 2;
    var wide = x(Math.min(d.t1, o.xMax)) - x(Math.max(d.t0, o.xMin));
    if (wide < 26) { return; }
    var lb = svgEl('text', { class: 'axis axis--day', x: x(mid), y: H - 6, 'text-anchor': 'middle' });
    /* 幅が足りないときは曜日を落とす */
    lb.textContent = wide >= 62 ? fmtDate(d.date) : fmtDateShort(d.date);
    svg.appendChild(lb);
  });

  /* 時刻の目盛り */
  var tick = hourTicks((o.xMax - o.xMin) / 3600e3, W - PL - PR);
  var stepH = tick.step;
  var withUnit = tick.unit;
  var j0 = new Date(o.xMin + 9 * 3600e3);
  j0.setUTCMinutes(0, 0, 0);
  while (j0.getUTCHours() % stepH !== 0) { j0.setTime(j0.getTime() + 3600e3); }
  for (var t = j0.getTime() - 9 * 3600e3; t <= o.xMax; t += stepH * 3600e3) {
    if (t < o.xMin) { continue; }
    var hh = new Date(t + 9 * 3600e3).getUTCHours();
    if (hh !== 0) {
      svg.appendChild(svgEl('line', { class: 'grid', x1: x(t), x2: x(t), y1: PT, y2: H - PB, opacity: '.6' }));
    }
    /* 1日刻みのときは全部0時になるので、下の日付だけにまかせます */
    if (stepH >= 24) { continue; }
    var lb2 = svgEl('text', { class: 'axis', x: x(t), y: H - PB + 14, 'text-anchor': 'middle' });
    lb2.textContent = withUnit ? hh + '時' : String(hh);
    svg.appendChild(lb2);
  }

  /* いまの時刻 */
  var nowT = Date.now();
  if (nowT > o.xMin && nowT < o.xMax) {
  ...
  o.series.forEach(function (s) {
    ...
    if (s.points.length === 1 || s.single) {
      s.points.forEach(function (p) {
        svg.appendChild(svgEl('circle', {
          cx: x(p.t).toFixed(1), cy: y(p.v).toFixed(1), r: 5, fill: s.color, stroke: 'none'
        }));
      });
      return;
    }
...
/* 気温のグラフに重ねる、日ごとの最高・最低・天気（気象庁の予報） */
function dailyMarks(xMin, xMax) {
  var marks = [];
  var w = buildWeekly();
  daysInRange(xMin, xMax).forEach(function (d) {
    var j = w.byKey.jma[d.key];
    if (!j) { return; }
...
function renderChart() {
  ...
  var now = Date.now();
  var xMin = now - PAST_HOURS * 3600e3;
  ...
  groups.forEach(function (g) {
    ...
    var mark = el('span', g.single ? 'legend__d' : 'legend__s');
...
function buildWeekly() {
  var byKey = { jma: {}, model: dailyFromHourly(state.future.model || []),
                ecmwf: dailyFromHourly(state.future.ecmwf || []),
                yahoo: {}, weathernews: {} };
...
function renderWeekly() {
  ...
  if (!w.days.length) {
    ...
    td.colSpan = SOURCE_KEYS.length + 1;
    tr.appendChild(td);
    body.appendChild(tr);
  ...
  var have = SOURCE_KEYS.filter(function (k) {
  ...
  $('weekly-note').textContent =
    ...
    '気象庁MSM/GSMの降水確率だけは、気象庁のモデルに含まれていないため ' +
    'Open-Meteo の総合予報（各社のモデルをまとめた値）を使っています。' +
    '取得できた提供元：' + have.join('・') + '。';
}
...
function renderStatus() {
  var ul = $('status-list');
  ul.innerHTML = '';
  var order = ['jma', 'jma_forecast', 'model', 'ecmwf', 'yahoo', 'weathernews', 'snapshot'];
  ...
  return loadAreas()
    ...
    .then(function (hit) {
      var jobs = [
        ...
        loadModel(state.place, 'model', 'jma_seamless'),
        loadModel(state.place, 'ecmwf', 'ecmwf_ifs025'),
        loadSnapshot(),
        loadAccuracy()
      ];
      if (hit) {
